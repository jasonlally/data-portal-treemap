...
for p in range(4, 13):
    url = URL('https://data.sfgov.org/browse?limitTo=datasets&sortBy=oldest&utf8=%E2%9C%93&page=' + str(p))
...
## elements to scrape each dataset.
    for i in dom.by_tag('tbody')[0:]:
        for g in i.by_tag('tr')[0:]:
            for h in g.by_tag('a.nameLink')[0:]:
                name = h.content
                name = plaintext(name)
            ...
            for link in g.by_tag('a')[0:1]:
                links = abs(link.attributes.get('href', ''), base=url.redirect or url.string)
                print links
                ff.get(links+"/about")
                element = ff.find_element_by_class_name("row_count")
                category_dt = ff.find_element_by_xpath("//div[@id='aboutSection']/div/div/div/div[4]/dl/dd[1]")
                print category_dt.text
                time.sleep(8)
                category = category_dt.text
